...
public:
    ...
    void Post(QueuedTask&& task) RTC_LOCKS_EXCLUDED(lock_) override;
    void PostDelayed(TimeDelta delay, QueuedTask&& task) RTC_LOCKS_EXCLUDED(lock_) override;
...
private:
    ...
    using ReadyTaskDeque = std::deque<QueuedTask>;
    ReadyTaskDeque ready_tasks_ RTC_GUARDED_BY(lock_);
    using DelayedTaskMap = std::map<Timestamp, std::vector<QueuedTask>>;
