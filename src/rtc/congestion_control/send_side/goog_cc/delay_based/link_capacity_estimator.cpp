#include "rtc/congestion_control/send_side/goog_cc/delay_based/link_capacity_estimator.hpp"

namespace naivertc {
namespace {

constexpr int kEstimatedStdDevBoundScale = 3;
...
std::optional<DataRate> LinkCapacityEstimator::UpperBound() const {
    if (estimate_kbps_) {
        ...
        // max bitrate.
        return DataRate::KilobitsPerSec(estimate_kbps_.value() + kEstimatedStdDevBoundScale * EstimatedStdDev());
    }
    return std::nullopt;
}
...
std::optional<DataRate> LinkCapacityEstimator::LowerBound() const {
    if (estimate_kbps_) {
        ...
        return DataRate::KilobitsPerSec(std::max<double>(0.0, estimate_kbps_.value() - kEstimatedStdDevBoundScale * EstimatedStdDev()));
    }
    return std::nullopt;
}
...
void LinkCapacityEstimator::Update(DataRate capacity_sample, double smoothing_coeff) {
    ...
    variance_kbps_ = std::min<double>(std::max<double>(variance_kbps_, 0.4f), 2.5f);
}

double LinkCapacityEstimator::EstimatedStdDev() const {
    assert(estimate_kbps_);
