...
MY_TEST(AimdRateControlTest, BweLimitedByAckedBitrate) {
    auto states = CreateAimdRateControlStates();
    constexpr int kAckedBitrate = 10000; // 10kbps
    constexpr double throughput_based_limit = 1.5 * kAckedBitrate + 10000 /* 10kbps*/;
    SetEstimate(states, kAckedBitrate);
    while (states.simulated_clock->now_ms() - kClockInitialTime < 20000 /* 20s */) {
        UpdateRateControl(states, BandwidthUsage::NORMAL, kAckedBitrate, states.simulated_clock->now_ms());
...
MY_TEST(AimdRateControlTest, BweNotLimitedByDecreaseingAckedBitrate) {
    auto states = CreateAimdRateControlStates();
    constexpr int kAckedBitrate = 100000; // 100kbps
    constexpr double throughput_based_limit = 1.5 * kAckedBitrate + 10000 /* 10kbps*/;
    SetEstimate(states, kAckedBitrate);
    while (states.simulated_clock->now_ms() - kClockInitialTime < 20000 /* 20s */) {
        UpdateRateControl(states, BandwidthUsage::NORMAL, kAckedBitrate, states.simulated_clock->now_ms());
    ...
    UpdateRateControl(states, BandwidthUsage::NORMAL, kAckedBitrate / 2, states.simulated_clock->now_ms());
...
MY_TEST(AimdRateControlTest, ExpectedPeriodAfter20kbpsDropAnd5kbpsIncrease) {
    ...
    UpdateRateControl(states, BandwidthUsage::OVERUSING, kAckedBitrate, states.simulated_clock->now_ms());
...
MY_TEST(AimdRateControlTest, BandwidthPeriodIsNotBelowMin) {
    auto states = CreateAimdRateControlStates();
    constexpr int kInitialBitrate = 10000; // 10 kbps
    SetEstimate(states, kInitialBitrate);
    states.simulated_clock->AdvanceTimeMs(100);
    // Make a small (1.5 kbps) bitrate drop to 8.5 kbps.
    UpdateRateControl(states, BandwidthUsage::OVERUSING, kInitialBitrate - 1, states.simulated_clock->now_ms());
...
MY_TEST(AimdRateControlTest, BandwidthPeriodIsNotAboveMaxNoSmoothingExp) {
    auto states = CreateAimdRateControlStates();
    constexpr int kInitialBitrate = 10010000; // 10010 kbps
    SetEstimate(states, kInitialBitrate);
    states.simulated_clock->AdvanceTimeMs(100);
...
MY_TEST(AimdRateControlTest, SendingRateBoundedWhenThroughputNotEstimated) {
    auto states = CreateAimdRateControlStates();
    constexpr int kInitialBitrate = 123000; // 123 kbps
    constexpr double throughput_based_limit = 1.5 * kInitialBitrate + 10000 /* 10kbps*/;
    UpdateRateControl(states, BandwidthUsage::NORMAL, kInitialBitrate, states.simulated_clock->now_ms());
...
MY_TEST(AimdRateControlTest, EstimateDoesNotIncreaseInAlr) {
    ...
    EXPECT_EQ(states.aimd_rate_control->LatestEstimate().bps(), kInitialBitrate);

    SetEstimate(states, 2 * kInitialBitrate);
    for (int i = 0; i < 100; ++i) {
        UpdateRateControl(states, BandwidthUsage::NORMAL, std::nullopt, states.simulated_clock->now_ms());
        states.simulated_clock->AdvanceTimeMs(100);
    }
    EXPECT_EQ(states.aimd_rate_control->LatestEstimate().bps(), 2 * kInitialBitrate);
}
...
MY_TEST(AimdRateControlTest, EstimateIncreaseWhileNotInAlr) {
    ...
    EXPECT_GT(states.aimd_rate_control->LatestEstimate().bps(), kInitialBitrate);
