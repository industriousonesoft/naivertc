...
void DtlsSrtpTransport::SendRtpPacket(std::shared_ptr<RtpPacket> rtc_packet, PacketSentCallback callback) {
    task_queue_.Post([this, rtc_packet=std::move(rtc_packet), callback](){
        auto raw_packet = rtc_packet->raw_packet();
        if (!raw_packet) {
            if (callback) {
                callback(false);
            }
        ...
        raw_packet->Resize(packet_size + SRTP_MAX_TRAILER_LEN);

        uint8_t payload_type = (raw_packet->bytes()[1]) & 0x7f;
        ...
        if (payload_type >= 64 && payload_type <= 95) {
            if (srtp_err_status_t err = srtp_protect_rtcp(srtp_out_, raw_packet->data(), &protectd_data_size)) {
                if (err == srtp_err_status_replay_fail) {
            ...
            if (srtp_err_status_t err = srtp_protect(srtp_out_, raw_packet->data(), &protectd_data_size)) {
                if (err == srtp_err_status_replay_fail) {
        ...
        raw_packet->Resize(protectd_data_size);

        if (raw_packet->dscp() == 0) {
            ...
            raw_packet->set_dscp(36);
        }

        Transport::Outgoing(raw_packet, callback);
    });
}
