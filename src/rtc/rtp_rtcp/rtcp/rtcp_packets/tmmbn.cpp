...
//  :                                                               :
// Temporary Maximum Media Stream Bit Rate Request (TMMBN) (RFC 5104).
// The Feedback Control Information (FCI) for the TMMBN
...
void Tmmbn::AddTmmbn(const TmmbItem& item) {
    items_.push_back(item);
}

size_t Tmmbn::PacketSize() const {
    return kRtcpCommonHeaderSize + kCommonFeedbackSize + TmmbItem::kFixedTmmbItemSize * items_.size();
}

bool Tmmbn::PackInto(uint8_t* buffer,
                   size_t* index,
                   size_t max_size,
