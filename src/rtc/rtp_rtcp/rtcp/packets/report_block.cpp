...
bool ReportBlock::PackInto(uint8_t* buffer, size_t size) const {
    ...
    ByteWriter<uint32_t>::WriteBigEndian(&buffer[8], extended_highest_seq_num_);
...
bool ReportBlock::Parse(const uint8_t* buffer, size_t size) {
    ...
    extended_highest_seq_num_ = ByteReader<uint32_t>::ReadBigEndian(&buffer[8]);
