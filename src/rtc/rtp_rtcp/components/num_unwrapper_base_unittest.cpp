#include "rtc/rtp_rtcp/components/num_unwrapper_base.hpp"
...
MY_TEST(NumUnwrapperBaseTest, UnwrapOddDivisor) {
...
MY_TEST(NumUnwrapperBaseTest, ManyForwardWraps) {
...
MY_TEST(NumUnwrapperBaseTest, ManyBackwardWraps) {
    const int kLargeNumber = 4711;
    const int kMaxStep = kLargeNumber / 2;
    const int kNumWraps = 100;
    NumberUnwrapper<uint16_t, kLargeNumber> unwrapper;
...
MY_TEST(NumUnwrapperBaseTest, UnwrapForward) {
    NumberUnwrapper<uint8_t> unwrapper;
    EXPECT_EQ(255, unwrapper.Unwrap(255));
...
MY_TEST(NumUnwrapperBaseTest, UnwrapForwardWithDivisor) {
    NumberUnwrapper<uint8_t, 33> unwrapper;
...
MY_TEST(NumUnwrapperBaseTest, UnwrapBackwards) {
    NumberUnwrapper<uint8_t> unwrapper;
...
MY_TEST(NumUnwrapperBaseTest, UnwrapBackwardsWithDivisor) {
    NumberUnwrapper<uint8_t, 33> unwrapper;
    EXPECT_EQ(0, unwrapper.Unwrap(0));
