...
std::vector<std::shared_ptr<RtpPacketToSend>> UlpFecGenerator::PopFecPackets() {
    ...
    auto last_media_packet = last_protected_media_packet_.value_or(nullptr);
    ...
    rtp_fec_packets.reserve(num_generated_fec_packets_);

    size_t total_fec_size_bytes = 0;
    for (size_t row = 0; row < num_generated_fec_packets_; ++row) {
