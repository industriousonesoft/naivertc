...
void InterFrameDelay::Reset() {
    ...
    prev_send_timestamp_ = 0;
    num_wrap_around_ = 0;
}

std::pair<int64_t, bool> InterFrameDelay::CalculateDelay(uint32_t send_timestamp, int64_t recv_time_ms) {
    if (prev_recv_time_ms_ == 0) {
        prev_recv_time_ms_ = recv_time_ms;
        prev_send_timestamp_ = send_timestamp;
        return {0, true};
    }
    ...
    num_wrap_around_ += wrap_around_utils::DetectWrapAround<uint32_t>(prev_send_timestamp_, send_timestamp);
    ...
    if ((wrap_arounds_since_prev == 0 && send_timestamp < prev_send_timestamp_) || wrap_arounds_since_prev < 0) {
    ...
    diff_timestamp_ = static_cast<int64_t>((send_timestamp + wrap_arounds_since_prev * kRoundTimestamp - prev_send_timestamp_) / 90.0 + 0.5);
