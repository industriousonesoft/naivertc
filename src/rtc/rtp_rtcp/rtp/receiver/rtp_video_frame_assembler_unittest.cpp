...
TEST_F(RTP_RTCP_PacketAssemblerTest, ClearSinglePacket) {
    ...
    frame_assembler_.ClearTo(seq_num);
...
TEST_F(RTP_RTCP_PacketAssemblerTest, ClearFullBuffer) {
    ...
    frame_assembler_.ClearTo(kMaxSize - 1);
...
TEST_F(RTP_RTCP_PacketAssemblerTest, DontClearNewerPacket) {
    ...
    EXPECT_THAT(Insert(3 * kStartSize + 1, kKeyFrame, kFirst, kNotLast).assembled_packets,
                IsEmpty());
    frame_assembler_.ClearTo(2 * kStartSize);
...
TEST_P(RTP_RTCP_PacketAssemblerH264ParameterizedTest, GetBitstreamBufferPadding) {
    ...
    packet->video_header.is_first_packet_in_frame = true;
    packet->video_header.is_last_packet_in_frame = true;
    auto frames = frame_assembler_.InsertPacket(std::move(packet)).assembled_packets;
...
TEST_P(RTP_RTCP_PacketAssemblerH264ParameterizedTest, FindFramesOnPadding) {
    ...
    EXPECT_THAT(InsertH264(2, kDeltaFrame, kFirst, kLast, 1000).assembled_packets,
                IsEmpty());
    EXPECT_THAT(frame_assembler_.InsertPadding(1), StartSeqNumsAre(2));
}

class PacketAssemblerH264XIsKeyframeTest : public PacketAssemblerH264Test {
protected:
    const uint16_t kSeqNum = 5;

    explicit PacketAssemblerH264XIsKeyframeTest(bool sps_pps_idr_is_keyframe)
        : PacketAssemblerH264Test(sps_pps_idr_is_keyframe) {}
