...
TEST(RtcpCommonHeaderTest, TooSmallBuffer) {
    ...
    CommonHeader header;
    EXPECT_FALSE(header.ParseFrom(buffer, 0));
    EXPECT_FALSE(header.ParseFrom(buffer, 1));
    EXPECT_FALSE(header.ParseFrom(buffer, 2));
    EXPECT_FALSE(header.ParseFrom(buffer, 3));
    EXPECT_TRUE(header.ParseFrom(buffer, 4));
}
...
TEST(RtcpCommonHeaderTest, Version) {
    ...
    buffer[0] = 0 << 6;
    EXPECT_FALSE(header.ParseFrom(buffer, sizeof(buffer)));
    buffer[0] = 1 << 6;
    EXPECT_FALSE(header.ParseFrom(buffer, sizeof(buffer)));
    buffer[0] = 2 << 6;
    EXPECT_TRUE(header.ParseFrom(buffer, sizeof(buffer)));
    buffer[0] = 3 << 6;
    EXPECT_FALSE(header.ParseFrom(buffer, sizeof(buffer)));
}
...
TEST(RtcpCommonHeaderTest, PacketSize) {
    ...
    CommonHeader header;
    EXPECT_FALSE(header.ParseFrom(buffer, sizeof(buffer) - 1));
    EXPECT_TRUE(header.ParseFrom(buffer, sizeof(buffer)));
...
TEST(RtcpCommonHeaderTest, PaddingAndPayloadSize) {
    ...
    EXPECT_FALSE(header.ParseFrom(buffer, 4));
