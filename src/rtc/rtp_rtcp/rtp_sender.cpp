...
bool RtpSender::EnqueuePackets(std::vector<std::shared_ptr<RtpPacketToSend>> packets) {
    ...
    for (auto& packet : packets) {
        ...
        if (!packet_sequencer_.AssignSequenceNumber(packet)) {
...
int32_t RtpSender::ResendPacket(uint16_t packet_id) {
    ...
    auto packet = packet_history_.GetPacketAndMarkAsPending(packet_id, [&](std::shared_ptr<RtpPacketToSend> stored_packet){
