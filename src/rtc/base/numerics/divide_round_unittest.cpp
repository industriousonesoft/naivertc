...
MY_TEST(DivideRoundUpTest, CanBeUsedAsConstexpr) {
    static_assert(DivideRoundUp(5, 1) == 5, "");
    static_assert(DivideRoundUp(5, 2) == 3, "");
}

MY_TEST(DivideRoundUpTest, ReturnsZeroForZeroDividend) {
...
MY_TEST(DivideRoundUpTest, WorksForMaxDividend) {
...
MY_TEST(DivideRoundToNearestTest, CanBeUsedAsConstexpr) {
...
MY_TEST(DivideRoundToNearestTest, DivideByOddNumber) {
...
MY_TEST(DivideRoundToNearestTest, DivideByEvenNumberTieRoundsUp) {
...
MY_TEST(DivideRoundToNearestTest, LargeDivisor) {
    EXPECT_EQ(DivideRoundToNearest(std::numeric_limits<int>::max() - 1,
                                 std::numeric_limits<int>::max()), 1);
}

MY_TEST(DivideRoundToNearestTest, DivideSmallTypeByLargeType) {
    uint8_t small = 0xff;
