...
bool BitReader::ReadExpGolomb(uint32_t& value) {
    ...
    if (value_bit_count > 32 || !PeekBits(value_bit_count, value)) {
...
bool BitReader::ReadExpGolomb(int32_t& value) {
...
bool BitReader::Seek(size_t byte_offset, size_t bit_offset) {
    if (byte_offset > byte_count_ || 
        bit_offset > 7 ||
        (byte_offset == byte_count_ && bit_offset_ > 0)) {
        return false;
    }
...
uint8_t BitReader::RightMostBits(uint8_t byte, size_t bit_count) {
    assert(bit_count <= 8);
    return byte & ((1 << bit_count) - 1);
}

// Returns the left-most `bit_count` bits in `byte`.
uint8_t BitReader::LeftMostBits(uint8_t byte, size_t bit_count) {
    assert(bit_count <= 8);
