...
public:
    RtpSender(const Configuration& config, 
...
private:
    ...
    std::unique_ptr<RtpPacketSentHistory> packet_history_;
    std::unique_ptr<RtpPacketSender> packet_sender_;
    std::unique_ptr<RtpPacketGenerator> packet_generator_;

    std::unique_ptr<NonPacedPacketSender> non_paced_sender_;
